...
`image.generateAsync()`, `image.removeBackgroundAsync()`, and `video.generateAsync()` return a PAT; rate, alt, and review do not. Treat it as optional: if signing fails the task is still created without one, so fall back to server-side polling. PATs are auto-generated by default. Disable them with `access: { pat: { enabled: false } }` when only server-side polling or webhooks are used.
